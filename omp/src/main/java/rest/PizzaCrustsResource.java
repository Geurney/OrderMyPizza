...
public class PizzaCrustsResource {
	...
	@Produces({ MediaType.TEXT_XML, MediaType.APPLICATION_XML,
			MediaType.APPLICATION_JSON })
	public List<PizzaCrust> getPizzaCrusts(@PathParam("token") String token) {
		List<PizzaCrust> pizzaCrusts = new ArrayList<PizzaCrust>();
		DatastoreService datastore = DatastoreServiceFactory
				.getDatastoreService();
	...
	/**
	 * Add/Update the Pizza Crust into PizzaFactory with form using token
	 * 
	 * @param token
	 *            Pizza Factory token
	 * @param identifier
	...
	@POST
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	public void newPostPizzaCrustXML(@PathParam("token") String token,
			@FormParam("identifier") String identifier,
			@FormParam("description") String description,
			@FormParam("cost1") String cost1,
			@FormParam("price1") String price1,
	...
	/**
	 * Add/Update the Pizza Crust into PizzaFactory with JSON using token
	 * 
	 * @param token
	 *            Pizza Factory token
	 * @param pizzaCrust
	...
	@POST
	@Consumes(MediaType.APPLICATION_JSON)
	public void newPostPizzaCrustJSON(@PathParam("token") String token,
	...
	@Path("/findbytoken/{token}")
	@DELETE
	public void deletePizzaCrusts(@PathParam("token") String token) {
		String hash_uid = UserUtils.getCurrentUserObscureID();
		if (hash_uid == null) {
			return;
		}
