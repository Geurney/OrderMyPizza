 ...
 *
 */
public class PizzaCrustsResource implements
		PizzaComponentsResourceInterface<PizzaCrust> {
	@Context
	UriInfo uriInfo;
	@Context
	Request request;
	 ...
	 */
	@Override
	public List<PizzaCrust> getComponents(@PathParam("token") String token) {
		return findPizzaComponents(token);
	}

	/**
	 * Add the Pizza Crust into PizzaFactory. This operation fails on following
	...
	/**
	 * Add the Pizza Crust into PizzaFactory with JSON. This operation fails on
	 ...
	 */
	@Override
	public void postPizzaComponent(PizzaCrust component) {
		String hash_uid = UserUtils.getCurrentUserObscureID();
	...
	/**
	 * Add the Pizza Crust into PizzaFactory with JSON with token for curl. This
	...
	/**
	 * Delete All Pizza Crusts with token
	 * 
	 * @param token
	 *            Pizza Factory token
	 */
	...
	private List<PizzaCrust> findPizzaComponents(String token) {
		if (token == null) {
			return null;
		}
		...
		try {
			...
			if (list != null) {
				for (EmbeddedEntity e : list) {
					PizzaCrust component = PizzaCrustResource.entityToObject(e); // DIFF
